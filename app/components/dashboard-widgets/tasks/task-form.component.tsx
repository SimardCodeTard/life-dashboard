...
export default function TaskForm({ onSubmit, mode, taskToEdit, setTaskToEdit }: NewTaskFormProps) {
    ...
    const [task, setTask] = mode === "edit" && taskToEdit ? [taskToEdit, setTaskToEdit] : useState<Task>(getDefaultTask());
    ...
    const onTaskDeadlineChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setTask({ ...task, deadline: DateTime.fromJSDate(new Date(e.target.value)) });
    }
